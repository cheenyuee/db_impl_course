...
bool match_join_condition(const Tuple *res_tuple,
    ...
    for (int i = 0; i < condition_idxs.size(); i++) {
        ...
        if (comp == EQUAL_TO && left_value.compare(right_value) != 0) {
            return false;
        }
        if (comp == NOT_EQUAL && left_value.compare(right_value) == 0) {
            return false;
        }
        if (comp == LESS_EQUAL && left_value.compare(right_value) > 0) {
            return false;
        }
        if (comp == LESS_THAN && left_value.compare(right_value) >= 0) {
            return false;
        }
        if (comp == GREAT_EQUAL && left_value.compare(right_value) < 0) {
            return false;
        }
        if (comp == GREAT_THAN && left_value.compare(right_value) <= 0) {
            return false;
        }
    }
    //
...
RC ExecuteStage::do_select(const char *db, Query *sql, SessionEvent *session_event) {
    ...
    if (tuple_sets.size() > 1) {
        ...
        for (int i = selects.attr_num - 1; i >= 0; i--) {
            for (int j = 0; j < old_schema.fields().size(); j++) {
                if (selects.attributes[i].relation_name == NULL
                    ...
                    const TupleField &field_now = old_schema.field(j);
        ...
        for (int i = 0; i < tuple_number; i++) {
            ...
            if (c)
                temp_tuples.push_back(std::move(merge_res));
        }
    } else {
        // 当前只查询一张表
        print_tuples.set_schema(tuple_sets.front().get_schema());
        for (auto &x: tuple_sets.front().tuples_) {
            temp_tuples.push_back(std::move(x));
        }
    }
    // 记录保存在 temp_tuples 中，对 temp_tuples 进行排序
        ...
        std::sort(temp_tuples.begin(), temp_tuples.end(), [&](Tuple &t1, Tuple &t2) {
            for (int i = 0; i < selects.order_num; i++) {
                ...
                for (int j = 0; j < (int)print_tuples.get_schema().fields().size(); j++) {
                    // orderDescription.relation_name == NULL 表示 order by column 写法
                    // orderDescription.relation_name == NULL 表示 order by table.column 写法
                ...
                if (comp != 0) {
                    if(orderDescription.type == kOrderAsc) { // 升序
                        return (int)(comp < 0);
                    } else {    // 降序
                        return (int)(comp >= 0);
                    }
                } else {
                    // 没有比较出结果，就循环继续对后面其他 field 进行比较
                    continue;
                }
            }
            return 1;
    ...
    // 将 temp_tuples 中的记录存入 print_tuples，并返回结果
