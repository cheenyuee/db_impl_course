...
Frame *BPManager::alloc(int file_desc, PageNum page_num) {
     ...
     */
    BufferTag buffer_tag(file_desc, page_num);


    if (!lrucache.exists(buffer_tag) and lrucache.size() < size) {
        int idx;
        ...
        lrucache.put(buffer_tag, idx);
    } else if(!lrucache.exists(buffer_tag) and lrucache.size() >= size) {
        BufferTag victim;
        lrucache.getVictim(&victim, not_pinned, (void *) (this));
        disk_buffer_pool->flush_block(get(victim.first, victim.second));
        lrucache.victim(victim, buffer_tag);
    }
    // printLruCache();
    return get(file_desc, page_num);
}
...
RC DiskBufferPool::allocate_page(int file_id, BPPageHandle *page_handle) {
    ...
    if ((tmp = allocate_block(file_handle->file_desc, file_handle->file_sub_header->page_count,
                              &(page_handle->frame))) != RC::SUCCESS) {
