...
RC Table::create(
    ...
    if (common::is_blank(name)) {   // 表名为空
        LOG_WARN("Name cannot be empty");
        return RC::INVALID_ARGUMENT;
    }
    LOG_INFO("Begin to create table %s:%s", base_dir, name);

    if (attribute_count <= 0 || nullptr == attributes) {    // 字段为空
...
RC Table::destroy(const char *dir) {
    ...
    /*
     * 使用 unlink 函数删除指定的文件。
     * 注意执行 unlink 函数并不一定会真正的删除文件，
     * 它先会检查文件系统中此文件的硬连接是否为1，如果不是1说明此文件还有其他链接对象，因此只对此文件的硬链接数进行减1操作，
     * 若硬链接数为1，并且在此时没有任何进程打开该文件，硬链接数减1后为0，此内容才会真正地被删除掉。
     */
